...
var contextform = document.querySelector(".context-form");

if (contextform){
  contextform.addEventListener("submit",(e)=>{
    e.preventDefault();
    var contextdescription=document.querySelector(".contextdescription");
    var contextcheckmark=document.querySelector(".contextcheckmark-updated");
    var contextcheckmarkerrorupdated=document.querySelector("#contextcheckmarkerrorupdated");
    var contextdescriptionerror=document.querySelector("#context-description");
  
    if (contextdescription.value.trim() === "") {
      contextdescription.style.border = "2px solid red";
      contextdescriptionerror.innerHTML = "Description Cannot be Empty";
    } else if (contextdescription.value.length < 5) {
      contextdescription.style.border = "2px solid red";
      contextdescriptionerror.innerHTML =
        "Title should be atleast 5 characters";
    } else {
      contextdescription.style.border = "2px solid green";
      headlinedescriptionerror.innerHTML = "";
    }
    if (!contextcheckmark.checked) {
      contextcheckmark.style.border = "2px solid red";
      contextcheckmarkerrorupdated.innerHTML = "Please Check Mark";
    } else {
      contextcheckmark.style.border = "2px solid green";
      contextcheckmarkerrorupdated.innerHTML = "";
    }
    if (contextdescriptionerror.innerHTML || contextcheckmarkerrorupdated.innerHTML){
      return false;
    }
    contextform.submit();
  });
